...
global.handleCommand = function(name, userid, text, source) {
	...
	if(text.toLowerCase().search(config.botinfo.botname + ' set avatar ') == 0){
		var command_length = (config.botinfo.botname + ' set avatar ').length;
		var avatar_id = text.substr(command_length, text.length - command_length);
		bot.setAvatar(parseInt(avatar_id));
	}

	if(text.toLowerCase() == 'list current playlist'){
		bot.playlistAll(function list_all(data){
			for(var i = 0; i < data.list.length; i++){
				output({text: (data.list[i].metadata.artist + ' - ' + data.list[i].metadata.song), destination:source, userid:userid});
			} 
		});
	}
